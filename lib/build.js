    ...
    mkdirp(outDir, function(err){
      ...
      ncp(fromDir, outDir, function(err){
        ...
        readHtml(from, function(err, $){
          ...
          async.forEachSeries(processors, process, afterProcess);

          function process(processor, cb){
          ...
          function afterProcess(err){
            if(err) return cb(err);
            var html = $.html();
            log.info('build', 'create file', newFile);
