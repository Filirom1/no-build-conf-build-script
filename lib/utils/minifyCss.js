...
return fullMatch;
    ...
    try {
      ...
      importContents = fs.readFileSync(fullImportFileName).toString(), i,
      ...
      console.log(fileName + "\n  Cannot inline css import, skipping: " + importFileName, e);
      return fullMatch;
    }
  });
}
...
*/
function cssFile (fileName, config) {
  var originalFileContents = fs.readFileSync(fileName).toString();
  var fileContents = flattenCss(fileName, originalFileContents, config.cssImportIgnore),
  startIndex, endIndex;
  ...
  try {
    ...
    console.error("Could not optimized CSS file: " + fileName + ", error: " + e);
  }
...
module.exports = function(fileName){
  log.info('css', 'minify css', fileName);
  return cssFile(fileName, { cssImportIgnore: false, optimizeCss: 'true' });
};
